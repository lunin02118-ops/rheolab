...
!macro NSIS_HOOK_POSTINSTALL
  ...
  IfFileExists "$INSTDIR\rheolab-app-icon.ico" 0 iconRefreshDone

  WriteRegStr SHCTX "${UNINSTKEY}" "DisplayIcon" "$\"$INSTDIR\rheolab-app-icon.ico$\""

  IfFileExists "$SMPROGRAMS\${PRODUCTNAME}.lnk" 0 skipStartMenuRoot
    CreateShortcut "$SMPROGRAMS\${PRODUCTNAME}.lnk" "$INSTDIR\${MAINBINARYNAME}.exe" "" "$INSTDIR\rheolab-app-icon.ico" 0
    !insertmacro SetLnkAppUserModelId "$SMPROGRAMS\${PRODUCTNAME}.lnk"
  skipStartMenuRoot:

  !if "${STARTMENUFOLDER}" != ""
    IfFileExists "$SMPROGRAMS\$AppStartMenuFolder\${PRODUCTNAME}.lnk" 0 skipStartMenuFolder
      CreateShortcut "$SMPROGRAMS\$AppStartMenuFolder\${PRODUCTNAME}.lnk" "$INSTDIR\${MAINBINARYNAME}.exe" "" "$INSTDIR\rheolab-app-icon.ico" 0
      !insertmacro SetLnkAppUserModelId "$SMPROGRAMS\$AppStartMenuFolder\${PRODUCTNAME}.lnk"
    skipStartMenuFolder:
  !endif

  IfFileExists "$DESKTOP\${PRODUCTNAME}.lnk" 0 skipDesktopShortcut
    CreateShortcut "$DESKTOP\${PRODUCTNAME}.lnk" "$INSTDIR\${MAINBINARYNAME}.exe" "" "$INSTDIR\rheolab-app-icon.ico" 0
