...
///
/// This is the **canonical** export entry-point.  It processes experiments in
/// batches of 50, keeping peak memory bounded to ~50 experiments × 3.5 MB ≈ 175 MB
/// regardless of total count.  The caller (TypeScript) reads the file via Tauri FS
/// and triggers a browser download.
///
/// The previous `experiments_export` command (non-streaming, full in-memory Vec<Value>)
/// has been removed in favour of this streaming variant — see the audit report
/// `docs/audit/REFACTORING_AUDIT_2026-04-18.md` § 2 (orphan command cleanup).
#[tauri::command]
pub async fn experiments_export_to_file(
    ...
    tracing::info!(
        "Export to file complete: {} experiments → {}",
        total,
        file_path_str
    );
