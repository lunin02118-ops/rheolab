...
pub fn parse_calibration_data(rows: &[Vec<String>]) -> Result<CalibrationReport, String> {
    let device_type =
        meta::detect_device_type(rows).ok_or("Не удалось определить тип калибровочного файла")?;
...
pub(super) fn analyze_quality(meta: &CalibrationMeta) -> Vec<String> {
    ...
    if meta.stdev >= CALIBRATION_LIMITS_STDEV {
        issues.push(format!(
            "STDEV ({:.2}) превышает предел {} dyne/cm²",
            meta.stdev, CALIBRATION_LIMITS_STDEV
        ));
    }

    if meta.hysteresis >= CALIBRATION_LIMITS_HYSTERESIS {
        issues.push(format!(
            "Гистерезис ({:.2}) превышает предел {} dyne/cm²",
            meta.hysteresis, CALIBRATION_LIMITS_HYSTERESIS
        ));
    }

    if meta.r_squared < CALIBRATION_LIMITS_R_SQUARED {
        issues.push(format!(
            "R² ({}) ниже минимального порога {}",
            meta.r_squared, CALIBRATION_LIMITS_R_SQUARED
        ));
    }
