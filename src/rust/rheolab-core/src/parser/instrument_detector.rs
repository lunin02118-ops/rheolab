...
pub fn detect_instrument(rows: &[Vec<String>], sheet_name: Option<&str>) -> Option<String> {
    ...
    // BSL (Prioritize over Chandler/Grace as it might contain similar keywords)
    if full_text.contains("bsl") || full_text.contains("rheometer model r1") ||
       full_text.contains("реометр модель: r1") || full_text.contains("model r1") ||
    ...
    if full_text.contains("brookfield") || full_text.contains("dv") ||
       full_text.contains("% torque") || full_text.contains("torque multiplier") ||
