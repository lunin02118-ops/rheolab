...
fn test_map_row_bsl_dropped_decimal_mode() {
    let row = vec!["1017".to_string(), "100".to_string(), "25".to_string()];
    let mapping = ColumnMapping {
        time_col: Some(0),
        viscosity_col: Some(1),
        temperature_col: Some(2),
...
fn test_detect_time_unit_no_false_positive_from_rpm() {
    ...
    let header = vec![
        "Время".to_string(),
        "Температура нагр".to_string(),
        "Температура образца".to_string(),
        "Давление".to_string(),
        "Угол".to_string(),
        "Напряж сдвига".to_string(),
        "Вязкость".to_string(),
        "Скорость".to_string(),
        "Ku".to_string(),
    ];
    ...
    assert!(
        !is_min,
        "Should NOT detect minutes from 'об/мин' in speed column"
    );
    assert!(!is_hr);
}
...
fn test_detect_pressure_multiplier_from_data_no_pressure_col() {
    ...
    let rows: Vec<Vec<String>> = vec![vec!["Time".to_string()], vec!["100.0".to_string()]];
    let result = detect_pressure_multiplier_from_data(&rows, 0, &mapping);
...
fn test_detect_time_unit_from_data_bsl_fractional_minutes() {
    ...
    let times = [
        "0.017", "0.1", "0.183", "0.267", "0.35", "0.433", "0.517", "0.6",
    ];
    for t in times {
    ...
    assert!(
        is_min,
        "Should detect fractional minutes for BSL-style time values"
    );
    assert!(!is_hr);
}
...
fn test_detect_time_unit_from_data_no_false_positive_seconds() {
    ...
    let mut rows: Vec<Vec<String>> = vec![vec!["Time".to_string(), "Viscosity".to_string()]];
    ...
    assert!(
        !is_min,
        "Should NOT trigger for seconds-scale time (first value = 5)"
    );
    assert!(!is_hr);
}
