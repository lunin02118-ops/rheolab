                ...
                .map(|e| {
                    ...
                    if let Some(src) = &source {
                        if let Some(range) = src.range(e.span) {
                            let text = src.text();
                            let line_num =
                                text[..range.start.min(text.len())].matches('\n').count() + 1;
                            let line_start = text[..range.start.min(text.len())]
                                .rfind('\n')
                                .map(|i| i + 1)
                                .unwrap_or(0);
                            let line_end = text[range.start.min(text.len())..]
                                .find('\n')
                                .map(|i| range.start + i)
                                .unwrap_or(text.len());
