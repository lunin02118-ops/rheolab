...
impl TypstWrapper {
    pub fn new(source_text: &str, files: HashMap<String, Vec<u8>>) -> Self {
        // Init Fonts
        let font_reg = Font::new(Bytes::from_static(FONT_ROBOTO), 0).expect("Failed to load Roboto Regular");
        let font_bold = Font::new(Bytes::from_static(FONT_ROBOTO_BOLD), 0).expect("Failed to load Roboto Bold");
...
pub fn compile_to_pdf(source_text: &str, files: HashMap<String, Vec<u8>>) -> Result<Vec<u8>, String> {
    ...
    if warned.output.is_err() {
        let errors = warned.output.as_ref().err().unwrap();
