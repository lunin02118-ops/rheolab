    ...
    match user {
        Some(ls) => {
            line.set_color(Color::RGB(parse_hex_to_rgb(&ls.color)))
                .set_width(ls.width as f64)
                .set_dash_type(style_to_dash_type(&ls.style));
        }
        None => {
            line.set_color(Color::RGB(default_rgb)).set_width(default_width);
...
fn add_series(
    ...
    series
        .set_name(name)
        .set_categories(("Report", 1, start_col, last_row, start_col))
        .set_values(("Report", 1, data_col, last_row, data_col))
