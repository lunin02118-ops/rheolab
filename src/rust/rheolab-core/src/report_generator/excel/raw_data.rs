...
pub(super) fn write_raw_data(
    ...
    for (idx, point) in input.raw_data.iter().enumerate() {
        ...
        if time_value > max_time_display {
            max_time_display = time_value;
        }
