...
fn produces_valid_xlsx_bytes() {
    let e1 = mk_entry(
        "e1",
        "Chandler",
        mk_input("T-1", vec![mk_point(0.0, 100.0), mk_point(300.0, 250.0)]),
    );
...
fn three_experiments_produce_workbook_with_six_sheets() {
    ...
    let entries = vec![
        mk_entry(
            "e1",
            "A",
            mk_input("T-1", vec![mk_point(0.0, 100.0), mk_point(60.0, 200.0)]),
        ),
        mk_entry(
            "e2",
            "B",
            mk_input("T-2", vec![mk_point(0.0, 150.0), mk_point(60.0, 250.0)]),
        ),
        mk_entry(
            "e3",
            "C",
            mk_input("T-3", vec![mk_point(0.0, 180.0), mk_point(60.0, 270.0)]),
        ),
    ];
...
fn experiment_name_with_forbidden_chars_is_sanitised() {
    let entries = vec![mk_entry(
        "e1",
        "Report/1[SST]:*?",
        mk_input("T-1", vec![mk_point(0.0, 100.0)]),
    )];
    let input = mk_comparison_input(entries);
    let bytes = generate_comparison_excel(&input).expect("generation");
    assert!(bytes.len() > 1000);
...
fn overlap_chart_sheet_has_chart_xml() {
    ...
    assert!(
        as_str.contains("xl/charts/chart1.xml"),
...
fn overlap_chart_with_threshold_generates_successfully() {
    let entries = vec![
        mk_entry(
            "e1",
            "A",
            mk_input("T-1", vec![mk_point(0.0, 100.0), mk_point(600.0, 500.0)]),
        ),
        mk_entry(
            "e2",
            "B",
            mk_input("T-2", vec![mk_point(0.0, 200.0), mk_point(600.0, 400.0)]),
        ),
    ];
...
fn mk_point_full(t: f64, v: f64, temp: f64, sr: f64, press: f64, bath: f64) -> DataPoint {
    DataPoint {
        ...
        shear_stress_pa: None,
        speed_rpm: None,
        pressure_bar: Some(press),
        bath_temperature_c: Some(bath),
    }
}
...
fn multiple_secondary_metrics_all_present() {
    ...
    assert!(
        bytes.len() > 5000,
        "multi-metric file must be substantial: {}",
        bytes.len()
    );
...
fn canonical_keys_are_normalised_for_excel() {
    ...
    assert_eq!(
        bytes_canonical, bytes_internal,
