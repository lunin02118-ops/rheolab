...
use super::super::super::chart_generator::{common::parse_hex_color, ChartConfig, ChartRanges};
...
pub(super) fn build_chart_full_page(
    ...
    let n_settings_left: usize =
        1 + if config.show_shear_rate && config.shear_rate_axis.trim().to_lowercase() == "left" {
            1
        } else {
            0
    ...
    if is_individual_mode {
        for axis in &chart_ranges.individual_axes {
            ...
            let title = match axis.metric.as_str() {
                ...
                "shear_rate" | "shearRate" => {
                    format!("{} (1/s)", escape_typst(&config.name_shear_rate))
                }
                "bath_temperature" | "bathTemperature" => {
                    format!("{} (°C)", escape_typst(&config.name_bath_temperature))
                }
        ...
        ticks_overlay.push_str(&make_ticks(
            chart_ranges.y_left_min,
            chart_ranges.y_left_max,
            chart_ranges.y_left_step,
            "left",
            left_axis_px,
            "",
        ));
        ...
        if has_right_axis {
            ticks_overlay.push_str(&make_ticks(
                chart_ranges.y_right_min,
                chart_ranges.y_right_max,
                chart_ranges.y_right_step,
                "right",
                right_axis_px,
                "",
            ));
