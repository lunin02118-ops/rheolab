...
fn renders_two_experiment_svg_with_expected_colours() {
    ...
    assert!(
        svg.contains("#FF0000"),
        "expected first experiment colour (red)"
    );
    assert!(
        svg.contains("#008000"),
        "expected second experiment colour (green)"
    );
...
fn errors_when_all_experiments_empty() {
    ...
    let err = generate_multi_experiment_chart_svg(&experiments, &mk_config()).unwrap_err();
...
fn single_experiment_renders_like_shared_single() {
    ...
    assert!(
        svg.contains("#3B82F6"),
        "single-exp colour should be present"
    );
}
...
fn downsamples_when_skip_flag_false() {
    ...
    assert!(
        svg.len() < 500_000,
        "SVG should be downsampled; got {} bytes",
        svg.len()
    );
}
...
fn dashed_bath_temp_does_not_leak_to_same_coloured_viscosity() {
    ...
    let (svg, _) =
        generate_multi_experiment_chart_svg(&experiments, &cfg).expect("render should succeed");
    ...
    for chunk in svg.split("<polyline").skip(1) {
        ...
        let tag_end = chunk
            .find("/>")
            .unwrap_or_else(|| chunk.find(">").unwrap_or(chunk.len()));
