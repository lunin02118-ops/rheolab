...
pub(super) fn render(
    ...
    let mut left_vals: Vec<f64> = points
        .iter()
    ...
    let (y_left_min_nice, y_left_max_nice, y_left_step, y_left_minor_step) =
        calculate_nice_scale(l_min_raw, l_max_raw, 12, true);
    let (y_right_min_nice, y_right_max_nice, y_right_step, y_right_minor_step) =
    ...
    let mut dash_styles: Vec<(String, u32, String)> = Vec::new(); // (rgb_string, width, style)
    {
...

        {
            ...
            let visc_data: Vec<(f64, f64)> = points
                .iter()
        ...
        let data_to_px_x =
            |dx: f64| -> i32 { (tm_left + (dx - x_min) / (x_max - x_min) * chart_w) as i32 };
