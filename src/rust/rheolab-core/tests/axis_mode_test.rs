...
fn pdf_generates_successfully_in_individual_axis_mode() {
    ...
    assert!(
        result.is_ok(),
        "PDF generation failed in individual mode: {:?}",
        result.err()
    );
    let bytes = result.unwrap();
    assert!(
        bytes.starts_with(b"%PDF-"),
        "Output is not a valid PDF in individual mode"
    );
}
...
fn pdf_generates_successfully_in_shared_axis_mode() {
    ...
    assert!(
        result.is_ok(),
        "PDF generation failed in shared mode: {:?}",
        result.err()
    );
    let bytes = result.unwrap();
    assert!(
        bytes.starts_with(b"%PDF-"),
        "Output is not a valid PDF in shared mode"
    );
}
...
fn pdf_generates_successfully_without_axis_mode_field() {
    ...
    assert!(
        result.is_ok(),
        "PDF generation failed without axis_mode: {:?}",
        result.err()
    );
    let bytes = result.unwrap();
...
fn excel_generates_successfully_in_individual_axis_mode() {
    ...
    assert!(
        result.is_ok(),
        "Excel generation failed in individual mode: {:?}",
        result.err()
    );
    let bytes = result.unwrap();
    ...
    assert!(
        ...
        "Output is not a valid XLSX in individual mode"
    );
}
...
fn excel_generates_successfully_in_shared_axis_mode() {
    ...
    assert!(
        result.is_ok(),
        "Excel generation failed in shared mode: {:?}",
        result.err()
    );
    let bytes = result.unwrap();
    assert!(
        bytes.starts_with(b"PK"),
        "Output is not a valid XLSX in shared mode"
    );
}
...
fn shear_rate_on_left_in_individual_mode_generates_valid_pdf() {
    ...
    assert!(
        result.is_ok(),
        "PDF failed with shear_rate_axis='left' + individual: {:?}",
        result.err()
    );
...
fn shear_rate_on_right_in_individual_mode_generates_valid_pdf() {
    ...
    assert!(
        result.is_ok(),
        "PDF failed with shear_rate_axis='right' + individual: {:?}",
        result.err()
    );
    assert!(result.unwrap().starts_with(b"%PDF-"));
}
...
fn shear_rate_on_left_in_shared_mode_generates_valid_pdf() {
    ...
    assert!(
        result.is_ok(),
        "PDF failed with shear_rate_axis='left' + shared: {:?}",
        result.err()
    );
    assert!(result.unwrap().starts_with(b"%PDF-"));
}
...
fn shear_rate_on_right_in_shared_mode_generates_valid_pdf() {
    ...
    assert!(
        result.is_ok(),
        "PDF failed with shear_rate_axis='right' + shared: {:?}",
        result.err()
    );
    assert!(result.unwrap().starts_with(b"%PDF-"));
}
...
fn shear_rate_left_draws_valid_svg_without_right_axis_line() {
    ...
    assert!(
        ranges.y_right_max <= 110.0,
        "Right scale should use default range (no data on right), got {}",
        ranges.y_right_max
    );
}
...
fn shear_rate_right_puts_values_on_right_scale() {
    ...
    assert_eq!(
        sr_axis.side, "right",
        "shear_rate must be on the right side"
    );
    assert!(
        sr_axis.max >= 170.0,
        "Shear rate axis max must cover value 170, got {}",
        sr_axis.max
    );
}
...
fn axis_mode_roundtrip_through_json() {
    ...
    let individual_json =
        make_report_input(r#""axis_mode": "individual", "shear_rate_axis": "left""#);
    let parsed_individual: ReportInput =
        serde_json::from_str(&individual_json).expect("parse failed");
...
fn chart_page_margins_identical_between_axis_modes() {
    ...
    let individual_json = make_report_input(&format!(
        r#""axis_mode": "individual", {settings_extra}"#,
        settings_extra = settings_extra.trim_matches(|c| c == ' ' || c == '\n' || c == ',')
    ));
    let shared_json = make_report_input(&format!(
        r#""axis_mode": "shared", {settings_extra}"#,
        settings_extra = settings_extra.trim_matches(|c| c == ' ' || c == '\n' || c == ',')
    ));

    let ind_pdf = generate_pdf_report(&individual_json).expect("individual mode PDF must succeed");
    let sha_pdf = generate_pdf_report(&shared_json).expect("shared mode PDF must succeed");

    assert!(
        ind_pdf.starts_with(b"%PDF"),
        "individual mode output must be a PDF"
    );
    assert!(
        sha_pdf.starts_with(b"%PDF"),
        "shared mode output must be a PDF"
    );
...
fn svg_chart_body_margins_match_between_modes() {
    ...
    let points = vec![
        ChartPoint {
            ...
            shear_rate: Some(50.0),
            pressure_bar: None,
            bath_temperature_c: Some(24.0),
        },
        ChartPoint {
            time_min: 5.0,
            viscosity_cp: 900.0,
    ...
    let (svg_sha, _ranges_sha) =
        generate_chart_svg(&points, &base_config("shared")).expect("shared SVG must succeed");
    ...
    let right_axes: Vec<_> = ranges_ind
        .individual_axes
        .iter()
    ...
    let left_axes: Vec<_> = ranges_ind
        .individual_axes
        .iter()
