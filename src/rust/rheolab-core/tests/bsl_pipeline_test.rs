...
fn test_bsl_swb_90_parse() {
    ...
    match &result {
        Ok(r) => {
            ...
            assert!(
                r.data.len() > 100,
                "Expected significant number of data points"
            );
            ...
            let has_rpm = r
                .data
                .iter()
                .any(|p| p.rpm.is_some() && p.rpm.unwrap() > 0.0);
            println!("[BSL] Has RPM data: {}", has_rpm);
            assert!(
                has_rpm,
                "BSL file should have RPM data from 'Скорость' column"
            );
        }
        Err(e) => {
...
fn test_bsl_swb_90_enforce_physics() {
    ...
    let data = fs::read(&path).expect("Failed to read BSL fixture");
    let mut result =
        parse_rheo_data(&data, "90 второй 26.02.2024 1717.da.xlsx").expect("Parse failed");

    let geometry = result.metadata.geometry.clone();
    println!(
        "[BSL] Pre-physics: {} points, geometry: {:?}",
        result.data.len(),
        geometry
    );

    // Check SR before enforce_physics
    let sr_count_before = result
        .data
        .iter()
        .filter(|p| p.shear_rate.is_some() && p.shear_rate.unwrap() > 0.0)
        .count();
    println!(
        "[BSL] Points with SR before physics: {}/{}",
        sr_count_before,
        result.data.len()
    );
    ...
    println!(
        "[BSL] Physics: sr_recovered={}, rpm_corrected={}",
        physics_result.sr_recovered, physics_result.rpm_corrected
    );

    let sr_count_after = result
        .data
        .iter()
        .filter(|p| p.shear_rate.is_some() && p.shear_rate.unwrap() > 0.0)
        .count();
    println!(
        "[BSL] Points with SR after physics: {}/{}",
        sr_count_after,
        result.data.len()
    );
    ...
    assert!(
        sr_count_after > sr_count_before,
        "enforce_physics should recover shear rates"
    );

    // Show unique SR values
    let mut unique_srs: Vec<f64> = result
        .data
        .iter()
    ...
    unique_srs.dedup();
    println!(
        "[BSL] Unique SR values (rounded to 0.1): {:?}",
        &unique_srs[..std::cmp::min(20, unique_srs.len())]
    );

    // Show first 5 points after physics
    for (i, p) in result.data.iter().take(5).enumerate() {
        println!(
            "[BSL] Post-physics Point {}: time={:.1}s, visc={:.1}, SR={:?}, SS={:?}, RPM={:?}",
...
fn test_bsl_swb_90_schedule_detection() {
    ...
    assert!(
        steps.len() > 1,
        "Should detect multiple steps in a multi-RPM BSL file"
    );
}
...
fn test_bsl_swb_90_cycle_detection() {
    ...
    println!(
        "[BSL] Detected {} cycles via anchor detection:",
        cycles.len()
    );
...
fn test_bsl_swb_90_full_pipeline() {
    ...
    let geometry_str = result
        .metadata
        .geometry
        .clone()
        .unwrap_or_else(|| "R1B5".to_string());
    ...
    println!(
        "[BSL Full Pipeline] {} points → {} steps → {} cycles",
        result.data.len(),
        steps.len(),
        cycles.len()
    );
    ...
    for (i, cycle) in cycles.iter().enumerate() {
        ...
        if data_points.len() >= 2 {
            ...
            match calculate_grace_internal(&data_points, &geometry_str, &settings, &cycle_info) {
                Some(grace) => {
                    println!(
                        "    Grace result: n'={:.4}, K'={:.4}",
                        grace.n_prime, grace.kv_pasn
                    );
                }
                None => {
