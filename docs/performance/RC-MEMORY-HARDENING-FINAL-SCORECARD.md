...
`src/lib/version.ts` is build-generated by `npm run version:sync`. The script
uses `git rev-parse --short HEAD` at build time, and both `prebuild:ci` and the
Tauri build path rerun it before producing artifacts. Therefore the committed
`COMMIT_HASH` is a provenance snapshot from the last sync, not a stable invariant
that can equal the final merge commit. Re-syncing and committing it would create
a new commit whose hash differs again.

For release artifacts, run the normal build gate; the generated bundle will
contain the current build HEAD hash. `npm run version:validate` remains the
release gate for SSoT version/channel consistency.
