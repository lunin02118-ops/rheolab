...
| **Real PDF export** | `reports_generate_comparison_pdf_by_ids` возвращает корректный PDF (magic bytes, > 5 KB). |
| **Real XLSX export** | `reports_generate_comparison_excel_by_ids` возвращает корректный XLSX (PK, > 5 KB). |
