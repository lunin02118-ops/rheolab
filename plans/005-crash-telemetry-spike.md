...
- ~~В `Cargo.toml` обнаружен `panic = "abort"` для release-профиля~~ —
  **РАЗРЕШЕНО 2026-06-12** (см. «Поправка 2026-06-12» ниже): условие
  сработало, согласовано с советником, исполнение продолжается с
  обязательными поправками. Это условие больше НЕ является STOP.
...
## Поправка 2026-06-12: разрешение STOP-условия panic = "abort"

Факт: `src-tauri/Cargo.toml:82` содержит `panic = "abort"`, строка 86 —
`strip = "symbols"` (release-профиль). Согласовано: ограничение совместимо
с дизайном — `std::panic::set_hook` исполняется ДО abort, запись crash.log
успевает. Обязательные поправки к исполнению:

1. **Шаг 1, `write_crash_report`**: перед возвратом из функции выполнить
   `file.sync_all()` (результат игнорировать, best effort) — после возврата
   из хука процесс абортится немедленно, полагаться на Drop-флаш нельзя.
2. **Шаг 4, design-док** — добавить три пункта:
   - release panic hook рассчитан на вызов до abort; отправка не реализуется;
   - первичный сигнал crash-отчёта — panic message + `location()`
     (compile-time строки, переживают strip); backtrace в release —
     best-effort адреса из-за `strip = "symbols"`;
   - открытый вопрос владельцу: сохранять ли PDB релизных сборок (MSVC
     кладёт символы в отдельный .pdb, пользователям он не уходит) для
     офлайн-символикации адресов из crash-отчётов.
3. **Тест-план без изменений**: test-профиль использует `panic = "unwind"`,
   inline-тесты writer/prune валидны как написаны. Abort-путь release
   отдельно не верифицируется (достаточно пометки в design-доке).
